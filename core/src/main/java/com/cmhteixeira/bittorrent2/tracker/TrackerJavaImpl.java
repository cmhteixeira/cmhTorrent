...
public class TrackerJavaImpl implements TrackerJava {
  ...
  public TrackerJavaImpl(Config config, TransactionIdGenerator txdIdGen, Executor ex)
  ...
  public void submit(InfoHash torrent, ImmutableList<UdpSocket> trackers) {
    ...
    ImmutableMap<InfoHash, State> newState =
        ImmutableMap.<InfoHash, State>builder()
            .putAll(currentState)
            .put(torrent, new State(ImmutableSet.of(), ImmutableMap.of()))
            .build();
      ...
      for (CompletableFuture<InetSocketAddress> fut : addTorrents(torrent, trackers)) {
        fut.thenApplyAsync(
                res -> {
                  logger.info(
                      String.format(
                          "Infinite loop for tracker '%s' on torrent '%s' finished.",
                          res, torrent));
                  return null;
                })
            .exceptionally(
                error -> {
                  logger.error(
                      String.format("Problem with a tracker for torrent '%s'.", torrent), error);
                  return null;
                });
      }
  }
  ...
  private ImmutableList<CompletableFuture<InetSocketAddress>> addTorrents(
    ...
    return trackers.stream()
        .map(
            udpSocket ->
                resolveHost(udpSocket)
                    .thenComposeAsync(
                        inetSocketAddress ->
                            run(torrent, inetSocketAddress).thenApply(a -> inetSocketAddress)))
  ...
  private CompletableFuture<InetSocketAddress> resolveHost(UdpSocket tracker) {
    ...
    return CompletableFuture.completedFuture(tracker)
        .thenComposeAsync(
            a -> {
              try {
                ...
                if (socketAddress.isUnresolved()) {
                  ...
                  return CompletableFuture.failedFuture(
                      new IllegalArgumentException(
                          String.format(
                              "Tracker '%s:%s' couldn't be resolved.",
            ...
            error -> {
              if (error instanceof CompletionException) {
                ...
                if (cause instanceof TimeoutException) {
                  logger.warn(
                      String.format(
                          "Obtaining peers from '$%s' from '%s'. Retrying ...", tracker, torrent),
                      cause);
  ...
  CompletableFuture<Pair<ConnectResponse, Long>> connect(
    ...
    if (state4Torrent == null) {
      ...
      if (!sharedState.compareAndSet(currentState, newState)) return connect(torrent, tracker, n);
        ...
        return promise
            .orTimeout(retries(Math.min(8, n)), TimeUnit.SECONDS)
            .exceptionallyComposeAsync(
                error -> {
  ...
  CompletableFuture<AnnounceResponse> announce(
    ...
    if (state4Torrent == null) {
      ...
      State newState4Torrent =
          new State(
              ...
              ImmutableMap.<InetSocketAddress, TrackerState>builder()
                  ...
                  .put(
                      tracker,
                      new TrackerState.AnnounceSent(txnId, connection.connectionId(), promise))
                  .build());
      ...
      if (!sharedState.compareAndSet(currentState, newState))
        return announce(torrent, tracker, connection, n);
      else {
                    ...
                    if (limitConnId(connection.timestamp)) {
                      return CompletableFuture.failedFuture(
                          new TimeoutException(
                              String.format(
                                  "Connection to %s (%s s) expired before announce received.",
                                  tracker, connAgeSec(connection.timestamp))));
      ...
      State newState4Torrent =
          new State(
              ...
              ImmutableMap.<InetSocketAddress, TrackerState>builder()
                  ...
                  .put(
                      tracker,
                      new TrackerState.AnnounceReceived(connection.timestamp, peers.size()))
                  .build());
      ...
      if (!sharedState.compareAndSet(currentState, newState))
        ...
        Executor newEx =
            CompletableFuture.delayedExecutor(
                config.announceTimeInterval.toMillis(), TimeUnit.MILLISECONDS, ex);
                ...
                a -> {
                  if (limitConnId(connection.timestamp)) {
                    return CompletableFuture.failedFuture(
                        new TimeoutException(
                            String.format(
                                "Connection to %s (%s s) expired.",
                                tracker, connAgeSec(connection.timestamp))));
                  } else {
                    logger.info(
                        String.format(
                            "Re-announcing to '%s' for '%s' Previous peers obtained: %s",
                            tracker, torrent, peers.size()));
                    return announce(torrent, tracker, connection, 0)
                        .thenComposeAsync(
                            announceResponse ->
                                setAndReannounce(
  ...
  private void sendConnectDownTheWire(int txnId, InetSocketAddress tracker) {
    ...
    try {
      ...
      logger.error(
          String.format("Sending connect message with txdId %s to %s", txnId, tracker), error);
    }
  }

  private void sendAnnounceDownTheWire(
      InfoHash infoHash, long conId, int txnId, InetSocketAddress tracker) {
    ...
    try {
      ...
      logger.error(
          String.format("Sending announce message with txdId %s to %s", txnId, tracker), error);
    }
  }
  ...
  private Tracker.Statistics statistics(State state) {
    Tracker.Statistics stats =
        new Tracker.Statistics(new Tracker.Summary(0, 0, 0, 0, 0), Map$.MODULE$.empty());
    for (Map.Entry<InetSocketAddress, TrackerState> trackerToState : state.trackers().entrySet()) {
      ...
      switch (trackerToState.getValue()) {
        ...
        case TrackerState.AnnounceReceived announceReceived -> stats =
            stats.addAnnounceReceived(tracker, announceReceived.numPeers());
        default -> throw new IllegalArgumentException("Never going to happen");
      }
    }
